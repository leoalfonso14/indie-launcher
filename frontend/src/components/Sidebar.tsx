...
export default function Sidebar() {
  ...
  return (
    <aside className="w-72 bg-carbon border-r border-slate-800 flex flex-col sticky top-0 h-screen relative z-20 overflow-hidden">
      ...
      <div className="p-8 space-y-12 relative z-10 flex flex-col h-full">
        ...
        <Link href="/" className="flex flex-col gap-1 group">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl overflow-hidden shadow-[0_0_20px_rgba(45,212,191,0.25)] transition-all duration-1000 ease-in-out group-hover:rotate-12 flex-shrink-0">
              <Image src="/logo.png" alt="IndieLauncher.AI Logo" width={40} height={40} className="w-full h-full object-cover" />
            </div>
            <div>
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-[10px] font-black text-slate-500 font-mono tracking-[0.2em] uppercase">Tactical OS</span>
              </div>
            </div>
          </div>
        </Link>
        ...
        <nav className="flex-1 space-y-8 overflow-y-auto pr-2 custom-scrollbar">
          <div className="space-y-2">
            <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] font-mono px-4">Modules</p>
              ...
              {menuItems.map((item) => (
                ...
                >
                  <item.icon className={cn(
                    ...
                    pathname === item.href ? item.color : "text-slate-600 group-hover:text-slate-400"
                  )} />
                  <span className="text-xs font-black tracking-widest">{item.title}</span>
                  {pathname === item.href && (
                    <div className={cn("ml-auto w-1.5 h-4 rounded-full", item.color.replace('text-', 'bg-'))} />
                  )}
                </Link>
              ))}
            </div>
          ...
          <div className="space-y-2">
            <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.3em] font-mono px-4">Account</p>
            <div className="space-y-1">
              <Link
                href="#"
              ...
              >
                ...
                <span className="text-xs font-black tracking-widest uppercase">Preferences</span>
              </Link>
              <button className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-slate-500 hover:text-red-400 hover:bg-red-500/5 transition-all font-mono cursor-pointer">
                ...
                <span className="text-xs font-black tracking-widest uppercase text-left">Logout</span>
              </button>
            </div>
          </div>
        </nav>
        ...
        <div className="mt-auto p-4 flex items-center gap-4 bg-white/5 rounded-2xl border border-white/5 transition-all hover:bg-white/10 group cursor-pointer">
          ...
          <div className="flex-1 overflow-hidden">
            <p className="text-[11px] font-black text-white truncate font-mono">Leandro Alfonso</p>
            <p className="text-[9px] text-slate-500 font-mono truncate uppercase tracking-widest">Pro Member</p>
          </div>
